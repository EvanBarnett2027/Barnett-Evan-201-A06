...
[https://github.com/EvanBarnett2027/Barnett-Evan-201-A06](https://github.com/EvanBarnett2027/Barnett-Evan-201-A06)
...
```{r}
points_per_game <- sapply(points_per_game, function(x) round(x,digits = 2))
points_per_game
```
...
```{r}
mat_rand <- apply(mat_rand, 1:2, function(x) round(x,digits = -nchar(as.character(x))+1))
mat_rand
```
...
max_temps <- as.data.frame(max_temps)
```
...
```{r}
median_annual_by_year <- c()
for (i in 1:nrow(CO)) {
  median_annual_by_year[i] <- median(unlist(CO[i,3:14]))
}

CO <- cbind(CO,median_annual_by_year)


most_year <- CO$year[which.max(CO$median_annual_by_year)]

least_year <- CO$year[which.min(CO$median_annual_by_year)]

print(paste0("The maximum median annual temperature of ", max(median_annual_by_year), " occured in ", most_year, " and the minimum median annual temperature of ", min(median_annual_by_year), " occured in ", least_year, "."))
```
